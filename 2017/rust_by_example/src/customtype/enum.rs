// 使用されていないコードwによる警告を隠すアトリビュート
#![allow(dead_code)]

// 人間を分類するための`enum`を作成。要素の名前と型の情報の両方が
// 変数の内容を同定するために、使用されいていることに注目、つまり
// `Skinny != Fat`であり、`Height(i32) != Weight(i32)`である。
enum Person {
    // `enum`の要素は以下のように様々な型を取れる
    // ユニットライクな型
    Skinny,
    Fat,
    // タプル
...
// `Person`型のEnumを引数に取り、返り値のない関数。
fn inspect(p: Person) {
    // `match`が`enum`型を精査する場合、その型について網羅的でなくてはならない。
    match p {
...
fn main() {
    ...
    // `to_owned()`は文字列のスライスから、新しい所有権の`String`を作成。
